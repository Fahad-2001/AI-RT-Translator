...
async def process_audio(file: UploadFile, main_language: str = Form(...), other_language: str = Form(...)):
    try:
        ...
        if content_type == 'application/octet-stream' or not content_type.startswith('audio/'):
            logger.warning(
                f"Received potentially ambiguous or non-audio content type: {content_type}. Attempting as audio/ogg.")
        ...
        if response.prompt_feedback:
            logger.info(
                f"Finish reason: {response.prompt_feedback.finish_reason}. Safety ratings: {response.prompt_feedback.safety_ratings}")
        ...
        translation_language_code = response_json.get("translation_language",
                                                      DEFAULT_TTS_LANGUAGE_CODE)  # Use detected language or default
        ...
        if translation_audio_base64:
            ...
            logger.warning(
                "Translation audio not generated (either no translation text, unknown language, or TTS failed).")
...
async def translate_text(request: TranslationRequest = Body(...)):
    ...
    try:
        ...
        if response.prompt_feedback and response.prompt_feedback.block_reason:
            logger.error(f"Blocked by Gemini: {response.prompt_feedback.block_reason}")
            raise HTTPException(status_code=400, detail="Translation blocked by safety filters.")
